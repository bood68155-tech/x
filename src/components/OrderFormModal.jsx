...
export default function OrderFormModal({ isOpen, onClose, preselectedProduct }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-[#0a0a0a] border border-[#B38F6F]/10  shadow-2xl shadow-black/60 animate-fade-in-up">
        <button onClick={onClose} className="absolute top-4 right-4 z-10 p-2 text-[#B38F6F]/70 hover:text-[#F2F1ED] transition-colors  hover:bg-[#B38F6F]/5">
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
          </svg>
        </button>

        <div className="px-8 pt-8 pb-2 text-center border-b border-[#B38F6F]/[0.06]">
          <div className="inline-flex items-center justify-center w-14 h-14  border border-[#B38F6F]/10 bg-[#B38F6F]/5 mb-4">
        ...
        <form onSubmit={handleSubmit} className="px-8 py-6 space-y-5">
          {submitted ? (
            <div className="text-center py-10">
              <div className="inline-flex items-center justify-center w-16 h-16  bg-green-500/10 border border-green-500/20 mb-4">
            ...
            <>
              <div>
                ...
                <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder={t('orderNamePh')}
                  className={`w-full px-4 py-3 bg-[#B38F6F]/[0.04] border ${errors.name ? 'border-[#710014]/60' : 'border-[#B38F6F]/[0.10]'}  text-[#F2F1ED] text-sm placeholder-[#767168] focus:outline-none focus:border-[#B38F6F]/40 transition-all ${fontClass} ${ar ? 'text-base' : ''}`} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder={t('orderEmailPh')}
                    className={`w-full px-4 py-3 bg-[#B38F6F]/[0.04] border ${errors.email ? 'border-[#710014]/60' : 'border-[#B38F6F]/[0.10]'}  text-[#F2F1ED] text-sm placeholder-[#767168] focus:outline-none focus:border-[#B38F6F]/40 transition-all ${fontClass} ${ar ? 'text-base' : ''}`} />
                ...
                <div>
                  ...
                  <input type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder={t('orderPhonePh')}
                    className={`w-full px-4 py-3 bg-[#B38F6F]/[0.04] border ${errors.phone ? 'border-[#710014]/60' : 'border-[#B38F6F]/[0.10]'}  text-[#F2F1ED] text-sm placeholder-[#767168] focus:outline-none focus:border-[#B38F6F]/40 transition-all ${fontClass} ${ar ? 'text-base' : ''}`} />
              ...
              <div>
                ...
                <input type="text" name="service" value={formData.service} onChange={handleChange} placeholder={t('orderServicePh')}
                  className={`w-full px-4 py-3 bg-[#B38F6F]/[0.04] border ${errors.service ? 'border-[#710014]/60' : 'border-[#B38F6F]/[0.10]'}  text-[#F2F1ED] text-sm placeholder-[#767168] focus:outline-none focus:border-[#B38F6F]/40 transition-all ${fontClass} ${ar ? 'text-base' : ''}`} />
              ...
              <div>
                ...
                <textarea name="details" value={formData.details} onChange={handleChange} placeholder={t('orderDetailsPh')} rows={4}
                  className={`w-full px-4 py-3 bg-[#B38F6F]/[0.04] border ${errors.details ? 'border-[#710014]/60' : 'border-[#B38F6F]/[0.10]'}  text-[#F2F1ED] text-sm placeholder-[#767168] focus:outline-none focus:border-[#B38F6F]/40 resize-none transition-all ${fontClass} ${ar ? 'text-base' : ''}`} />
                {errors.details && <p className="text-xs text-[#B38F6F] mt-1">{t('orderRequired')}</p>}
              </div>

              <button type="submit" disabled={saving}
                className={`w-full py-4 bg-[#710014] text-[#F2F1ED] font-semibold text-sm  hover:bg-[#5F0B1E] transition-all duration-300 uppercase tracking-widest ${fontClass} ${ar ? 'tracking-normal normal-case text-base' : ''} disabled:opacity-50`}>
                {saving ? (
