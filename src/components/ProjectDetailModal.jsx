...
export default function ProjectDetailModal({ project, onClose, onBuy }) {
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal Window */}
      <div className="relative w-full max-w-5xl max-h-[85vh] bg-[#111116] border border-white/[0.08]  shadow-2xl shadow-black/80 overflow-hidden animate-fade-in-up flex flex-col">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-white/[0.06] bg-white/[0.02] shrink-0">
          ...
          <button onClick={onClose} className="p-1.5  text-[#c29b7f]/70 hover:text-white hover:bg-[#c29b7f]/[0.07] transition-all">
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl w-full">
            ...
            <div className="p-5 sm:p-6 border-b md:border-b-0 md:border-r border-white/[0.06]">
              {/* Main Media Preview */}
              <div className="relative  overflow-hidden border border-white/[0.08] bg-gradient-to-br from-[#111116] to-[#08080a] aspect-video">
                {allMedia.length > 0 ? (
                ...
                {hasVideo && allMedia[activeImage]?.type !== 'video' && (
                  <button onClick={() => setActiveImage(allMedia.findIndex(m => m.type === 'video'))}
                    className="absolute bottom-3 right-3 z-10 flex items-center gap-1.5 px-2.5 py-1.5  bg-[#08080a]/60 backdrop-blur-sm text-white text-xs font-medium hover:bg-[#08080a]/80 transition-all">
                ...
                <div className="flex gap-2 mt-3 overflow-x-auto pb-1">
                  {allMedia.map((media, idx) => (
                    <button key={idx} onClick={() => setActiveImage(idx)}
                      className={`relative flex-shrink-0 w-16 h-16  overflow-hidden border-2 transition-all duration-200 ${activeImage === idx ? 'border-[#c29b7f] scale-105' : 'border-white/10 hover:border-white/20 opacity-60 hover:opacity-100'}`}>
                      {media.type === 'video'
              ...
              <div className="mt-5 space-y-2.5">
                {hasDemo && (
                  <a href={project.demoUrl} target="_blank" rel="noopener noreferrer"
                    className="w-full py-3 bg-[#c29b7f] hover:bg-[#c29b7f] font-bold text-[#08080a] text-sm font-bold  transition-all flex items-center justify-center gap-2.5 uppercase tracking-wider">
                ...
                {hasSource && (
                  <a href={project.sourceCodeUrl} target="_blank" rel="noopener noreferrer"
                    className="w-full py-3 bg-[#c29b7f]/[0.05] hover:bg-[#c29b7f]/[0.10] text-white text-sm font-medium  border border-white/[0.12] hover:border-[#c29b7f]/[0.2] transition-all flex items-center justify-center gap-2.5 uppercase tracking-wider">
                ...
                {hasVideo && allMedia[activeImage]?.type !== 'video' && (
                  <button onClick={() => setActiveImage(allMedia.findIndex(m => m.type === 'video'))}
                    className="w-full py-3 bg-[#c29b7f]/[0.05] hover:bg-[#c29b7f]/[0.10] text-white text-sm font-medium  border border-white/[0.12] hover:border-[#c29b7f]/[0.2] transition-all flex items-center justify-center gap-2.5">
            ...
            <div className="p-5 sm:p-6 flex flex-col">
              ...
              {project.tag && (
                <div className="flex items-center gap-2 mb-4">
                  <span className={`text-[10px] font-semibold uppercase tracking-wider px-2.5 py-1  bg-[#c29b7f]/[0.07] text-[#c29b7f] border border-white/[0.08] ${ar ? 'tracking-normal normal-case text-xs' : ''}`}>{project.tag}</span>
                </div>
              )}
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.features.map((feat, i) => (
                      <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5  bg-[#c29b7f]/[0.05] border border-white/[0.08] text-xs font-medium text-white/80">
                        <span className="w-1.5 h-1.5  bg-[#c29b7f]/70" />
                        {feat}
                      </span>
                    ))}
                  </div>
              ...
              {!showCheckout ? (
                <>
                  {/* Price Badge */}
                  {displayPrice ? (
                    <div className="flex items-center justify-between mb-5 p-4  bg-[#c29b7f]/[0.04] border border-white/[0.06]">
                      <span className="text-sm text-[#c29b7f]">Price</span>
                      <span className={`text-2xl font-bold text-white ${fontClass}`}>{displayPrice}</span>
                    </div>
                  ) : (
                    <div className="mb-5 p-4  bg-[#c29b7f]/[0.04] border border-white/[0.06] text-center">
                  ...
                  <button onClick={() => setShowCheckout(true)}
                    className="w-full py-4 bg-[#c29b7f] hover:bg-[#c29b7f] font-bold text-[#08080a] text-sm font-bold  transition-all uppercase tracking-wider flex items-center justify-center gap-2.5">
                  ...
                  <button onClick={() => setShowCheckout(true)}
                    className="w-full mt-2.5 py-3 bg-[#c29b7f]/[0.05] hover:bg-[#c29b7f]/[0.10] text-white/80 text-sm font-medium  border border-white/[0.10] hover:border-[#c29b7f]/[0.2] transition-all text-center">
                ...
                <div className="text-center py-6">
                  <div className="w-14 h-14  bg-[#c29b7f]/10 border border-white/20 flex items-center justify-center mx-auto mb-4">
                ...
                <div>
                  ...
                  <div className="p-3  bg-[#c29b7f]/[0.04] border border-white/[0.06] mb-4">
                  ...
                  <div className="flex gap-2 mb-4">
                    {['TRC20', 'BEP20'].map(n => (
                      <button key={n} onClick={() => setBinanceNetwork(n)}
                        className={`flex-1 py-2 text-[11px] font-semibold  border transition-all ${binanceNetwork === n ? 'bg-[#c29b7f]/10 border-yellow-500/30 text-yellow-400' : 'bg-[#c29b7f]/[0.04] border-white/[0.08] text-[#c29b7f]/70 hover:text-[#c29b7f]'}`}>{n}</button>
                    ))}
                  </div>
                  <div className="mb-4">
                    ...
                    <div className="flex items-stretch gap-1.5">
                      <div className="flex-1 px-3 py-2.5 bg-[#c29b7f]/[0.05] border border-white/10  text-[11px] text-white/80 font-mono break-all select-all leading-relaxed">{project.binanceWallet || BINANCE_WALLET_DEFAULT}</div>
                      ...
                      }} className={`shrink-0 px-3  text-[10px] font-semibold transition-all ${binanceCopied ? 'bg-[#c29b7f]/15 border border-[#c29b7f]/25 text-[#c29b7f]' : 'bg-[#c29b7f]/[0.07] border border-white/10 text-[#c29b7f] hover:text-white'}`}>
                        {binanceCopied ? '✓' : 'Copy'}
                      </button>
                    </div>
                  </div>
                  <ol className="space-y-1 text-[11px] text-[#c29b7f]/70 mb-4">
                    {['Open Binance app.', `Send ${numericTotal} USDT on ${binanceNetwork}.`, 'Copy TxID after sending.', 'Paste below and confirm.'].map((s, i) => (
                      <li key={i} className="flex items-start gap-2"><span className="w-4 h-4  bg-[#c29b7f]/[0.07] text-[#c29b7f]/70 flex items-center justify-center text-[8px] font-bold shrink-0 mt-0.5">{i + 1}</span>{s}</li>
                    ))}
                  </ol>
                  <form onSubmit={async (e) => {
                    ...
                    <input name="txHash" type="text" required placeholder="Paste TxID..."
                      className="w-full px-3 py-2.5 bg-[#c29b7f]/5 border border-white/10  text-white text-xs font-mono placeholder-[#71717a] focus:outline-none focus:border-[#c29b7f]/40 transition-all" />
                    <button type="submit" disabled={orderLoading}
                      className="w-full py-3 bg-[#800020] text-white text-xs font-bold  hover:bg-[#6b0c22] transition-all uppercase tracking-wider disabled:opacity-40">
                  ...
                  <div className="space-y-2">
                    {project.binancePayEnabled && (
                      <button onClick={() => setPaymentMethod('binance')}
                        className="w-full p-3.5  border border-yellow-500/10 bg-[#c29b7f]/[0.03] hover:bg-[#c29b7f]/[0.08] hover:border-yellow-500/25 transition-all flex items-center gap-3 text-left group">
                        <div className="w-10 h-10  bg-[#c29b7f]/[0.08] border border-yellow-500/15 flex items-center justify-center shrink-0">
                    ...
                    <button onClick={() => setPaymentMethod('crypto')}
                      className="w-full p-3.5  border border-white/[0.08] bg-white/[0.02] hover:bg-[#c29b7f]/[0.06] hover:border-[#c29b7f]/20 transition-all flex items-center gap-3 text-left group">
                      <div className="w-10 h-10  bg-[#c29b7f]/[0.06] border border-yellow-500/10 flex items-center justify-center shrink-0">
                    ...
                    <button onClick={() => setPaymentMethod('contact')}
                      className="w-full p-3.5  border border-white/[0.08] bg-white/[0.02] hover:bg-[#c29b7f]/[0.06] hover:border-[#c29b7f]/20 transition-all flex items-center gap-3 text-left group">
                      <div className="w-10 h-10  bg-[#c29b7f]/[0.07] border border-white/[0.08] flex items-center justify-center shrink-0 group-hover:border-[#c29b7f]/20 transition-all">
                ...
                <div>
                  ...
                  <form onSubmit={handleOrderSubmit} className="space-y-3">
                    <input type="text" required value={orderForm.name} onChange={e => setOrderForm({ ...orderForm, name: e.target.value })} placeholder="Full name"
                      className="w-full px-3 py-2.5 bg-[#c29b7f]/5 border border-white/10  text-white text-xs placeholder-[#71717a] focus:outline-none focus:border-[#c29b7f]/40 transition-all" />
                    ...
                    <button type="submit" disabled={orderLoading || !orderForm.name.trim() || !orderForm.email.trim()}
                      className="w-full py-3 bg-[#800020] text-white text-xs font-bold  hover:bg-[#6b0c22] transition-all uppercase tracking-wider disabled:opacity-40 flex items-center justify-center gap-2">
